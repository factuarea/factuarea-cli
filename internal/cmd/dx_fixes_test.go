...
func TestFileNotFoundTranslatedAndUsageExit(t *testing.T) {
	...
	for _, c := range cases {
		args := append([]string{"contacts", "create", "--company", "acme_co"}, c...)
		_, err := runCmd(t, "", args...)
		if err == nil {
...
func TestDataFileIsDirectoryTranslated(t *testing.T) {
	dir := t.TempDir()
	_, err := runCmd(t, "", "contacts", "create", "--company", "acme_co", "--data-file", dir)
...
func TestKeyValueParseErrorTranslated(t *testing.T) {
	_, err := runCmd(t, "", "contacts", "create", "--company", "acme_co", "--metadata", "sinformato", "--name", "x", "--kind", "person", "--roles", "customer", "--dry-run")
